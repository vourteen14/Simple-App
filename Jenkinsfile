node {
    ...
    stage("Buat Image"){
        ...
        sh 'docker tag simple-app vourteen14/simple-app:v3'
    }
    withCredentials([string(credentialsId: 'DOCKERHUB_PASSWD', variable: 'PASSWORD')]) {
        sh 'docker login -u vourteen14 -p $PASSWORD'
    }
    stage("Push Image"){
        sh 'docker push vourteen14/simple-app:v3'
    }
	stage("Deploy ke Kubernetes"){
		sh 'microk8s.kubectl set image simple-app-production simple-app-production=vourteen14/simple-app:v3'
	}
}
